...
const Home:React.FC = () => {
  ...
  useEffect(() => {
    ...
    t1
    .fromTo([modalRef.current], 1, { width: '100%' }, { width: '0%', delay: 5, ease: Expo.easeInOut })
    .fromTo([videoRef.current], 2, { width: '100%', opacity: 0 }, { width: '100%', opacity: 1, ease: Expo.easeInOut }, '-=1')
    .fromTo([logoRef.current], 0.7, { y: 50, opacity: 0 }, { y: 0, opacity: 1, ease: Expo.easeInOut }, '-=0.5')
    .fromTo([navListRef.current], 0.7, { y: 50, opacity: 0 }, { y: 0, opacity: 1, ease: Expo.easeInOut }, '-=0.5')
    .fromTo([navSocialRef.current], 0.7, { y: 50, opacity: 0 }, { y: 0, opacity: 1, ease: Expo.easeInOut }, '-=0.5')
    .fromTo([containerOneRef.current], 0.7, { y: 50, opacity: 0 }, { y: 0, opacity: 1, ease: Expo.easeInOut }, '-=0.5')
    .fromTo([textRef.current], 0.7, { y: 50, opacity: 0 }, { y: 0, opacity: 1, ease: Expo.easeInOut }, '-=0.5')
    .fromTo([buttonsRef.current], 0.7, { y: 50, opacity: 0 }, { y: 0, opacity: 1, ease: Expo.easeInOut }, '-=0.5')
    .fromTo([iconsRef.current], 0.7, { y: 50, opacity: 0 }, { y: 0, opacity: 1, ease: Expo.easeInOut }, '-=0.5')
    .fromTo([containerTwoRef.current], 0.7, { y: 50, opacity: 0 }, { y: 0, opacity: 1, ease: Expo.easeInOut }, '-=0.5')
  ...
  return (
    ...
    <header>
      <nav className="navbar">
        ...
        <div className="navbar-social" ref={navSocialRef}>
          <FaFacebook size={24} />
          <FaInstagram size={24} />
          <FaYoutube size={24} />
          <FaTwitter size={24} />
    ...
    <div className="bg-video" ref={videoRef}>
      <video 
        className="video"
        autoPlay
        muted
    ...
    <main className="container">
      <div className="container-item-1" ref={containerOneRef}>
        <section className="px-1">
          <img src={spiderText} className="spider-text" alt="Spider Man Text"/>
        </section>
        <p className="my-2" ref={textRef}>
